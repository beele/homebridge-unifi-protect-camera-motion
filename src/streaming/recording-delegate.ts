...
export class ProtectRecordingDelegate implements CameraRecordingDelegate {
    ...
    public rtspStream: Nullable<UnifiCameraStream>;
    ...
    public async updateRecordingActive(active: boolean): Promise<void> {
        ...
        this.log.info("HKSV: %s%s, %s kbps (%s second timeshift buffer).",
            this.ffmpegOptions.hardwareTranscoding ? "hardware-accelerated " : "",
            this.rtspStream?.name, this.recordingConfig?.videoCodec.parameters.bitRate.toLocaleString("en-US"), this.timeshift.configuredDuration / 1000);
    }
    ...
    private async startTransmitting(): Promise<boolean> {
        ...
        if (!this.recordingConfig || !this.rtspStream) {
            return false;
        }
    ...
    private stopTransmitting(reason?: HDSProtocolSpecificErrorReason): void {
        ...
        if (this.accessory.context.hksvRecording && this.timeshiftedSegments && this.transmittedSegments && this.rtspStream) {
            ...
            if ((reason === HDSProtocolSpecificErrorReason.NORMAL)) {
                this.log.debug("HKSV: %s %s event.", recordedTime, timeUnit);
            }
