...
export class LivestreamManager {
    ...
    constructor(unifi: Unifi | undefined, camera: UnifiCamera, logging: Logging) {
        this.log = logging;
    ...
    public acquire(channel: number, lens?: number): ProtectLivestream {
        if (!this.unifi) {
            throw new Error('Cannot create livestream API, unifi not set!');
        }
        ...
        return this.livestreams[index] = this.unifi.createLivestream();
    }
    ...
    public async start(channel: number, lens?: number, segmentLength = PROTECT_HKSV_SEGMENT_RESOLUTION): Promise<boolean> {
        ...
        if (!this.subscriberCount[index] &&
            !(await this.livestreams[index].start(this.camera.id, channel, lens, segmentLength, this.camera.name + ":" + index))) {
        ...
        if (!this.subscriberCount[index]) {
            ...
            this.livestreams[index].on("restart", this.eventHandlers[index + ".restart"] = async (): Promise<void> => {
                ...
                isRestarting = true;
                this.log.warn("Reconnecting to the livestream API.");
